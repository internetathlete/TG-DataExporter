...
def detect_language(client_path):
    ...
    try:
        ...
        if not location:
            location = pyautogui.locateOnScreen(
                os.path.join(SCREENSHOT_DIR, DEFAULT_LANGUAGE, "hamburger_menu_dark.png"),
                confidence=0.8
            )
...
def export_telegram_data(client_path, export_base_dir):
    ...
    try:
        # 使用subprocess启动客户端
        logging.info(f"启动客户端: {client_path}")
        ...
        for attempt in range(3):  # 尝试3次
            try:
                ...
                if not hamburger:
                    hamburger = pyautogui.locateOnScreen(
                        os.path.join(SCREENSHOT_DIR, language, "hamburger_menu_dark.png"),
                        confidence=0.8
                    )
        ...
        try:
            ...
            # 方法2: 使用taskkill强制关闭所有Telegram进程 - 使用更强力的方式
            try:
                # 使用/F强制关闭，/T终止所有子进程
                ...
                # 再次检查进程是否真的关闭了
                result = subprocess.run('tasklist /FI "IMAGENAME eq Telegram.exe" /NH', 
                                      shell=True, 
                                      stdout=subprocess.PIPE, 
                                      stderr=subprocess.PIPE,
                                      text=True)
                
                # 如果进程仍然存在，使用更强力的方法再次尝试关闭
                if "Telegram.exe" in result.stdout:
                    logging.warning("首次关闭未成功，使用更强力方法再次尝试")
                    
                    # 尝试获取进程ID并强制终止
                    pid_cmd = 'wmic process where name="Telegram.exe" get ProcessId'
                    pid_result = subprocess.run(pid_cmd, 
                                              shell=True, 
                                              stdout=subprocess.PIPE, 
                                              stderr=subprocess.PIPE,
                                              text=True)
                    ...
                    # 解析进程ID
                    if "ProcessId" in pid_result.stdout:
                        lines = pid_result.stdout.strip().split('\n')
                        if len(lines) > 1:  # 至少有标题行和一个进程ID
                            for line in lines[1:]:  # 跳过标题行
                                ...
                                if pid and pid.isdigit():
                                    ...
                                    subprocess.run(f"taskkill /F /PID {pid}", 
                                                  ...
                                                  stdout=subprocess.PIPE, 
                                                  stderr=subprocess.PIPE)
                                    logging.info(f"已强制终止进程ID: {pid}")
                    
                    # 最后再检查一次
                    time.sleep(2)
                    final_check = subprocess.run('tasklist /FI "IMAGENAME eq Telegram.exe" /NH', 
                                               shell=True, 
                                               stdout=subprocess.PIPE, 
                                               stderr=subprocess.PIPE,
                                               text=True)
                    
                    if "Telegram.exe" in final_check.stdout:
                        logging.error("无法完全关闭Telegram客户端，可能需要手动关闭")
                    else:
                        logging.info("已确认Telegram客户端已完全关闭")
                        close_success = True
                else:
                    logging.info("已确认Telegram客户端已完全关闭")
                    close_success = True
            except Exception as e:
                logging.debug(f"进程关闭失败: {str(e)}")
            ...
            try:
                ...
                time.sleep(1)
                # 使用Alt+Tab切换窗口焦点，可能有助于释放资源
                pyautogui.hotkey('alt', 'tab')
                time.sleep(0.5)
                # 再次显示桌面
                pyautogui.hotkey('win', 'd')
                logging.info("已显示桌面并切换窗口焦点，确保所有窗口不会干扰后续操作")
            except Exception as e:
...
# 添加一个新函数，用于GUI程序调用
def run_export(source_dir, export_dir, callback=None):
    """
    ...
    for idx, exe_path in enumerate(clients, 1):
        ...
        try:
            ...
            if export_success is False:  # 明确检查是否为False，因为未登录的情况下返回None
                message = f"警告：客户端 {client_dir} 未成功导出数据 (未找到'Show My Data'按钮)"
                if callback:
                    callback(message)
                logging.warning(message)
                failed_clients.append(os.path.dirname(exe_path))  # 保存完整路径
            elif export_success is None:  # 处理未登录或其他提前返回的情况
                message = f"警告：客户端 {client_dir} 未处理 (可能未登录或状态异常)"
                if callback:
                    callback(message)
                logging.warning(message)
                failed_clients.append(os.path.dirname(exe_path))  # 保存完整路径
            else:
                message = f"客户端 {client_dir} 数据导出成功"
                if callback:
                    callback(message)
                logging.info(message)
                success_clients.append(client_dir)  # 添加到成功列表
        except Exception as e:
            message = f"错误：客户端 {client_dir} 导出失败 - {str(e)}"
            if callback:
                callback(message)
...
def console_main():
    ...
    # 调用新的run_export函数
    result = run_export(source_dir, export_dir, callback=print)
...
if __name__ == "__main__":
    ...
    try:
        ...
        screenshot_prompts = [
            ...
            "👉 1. 菜单按钮 (hamburger_menu.png) - 左上角的三横线菜单按钮（白底样式）",
            "👉 1a. 菜单按钮 (hamburger_menu_dark.png) - 左上角的三横线菜单按钮（黑底样式，可选）",
